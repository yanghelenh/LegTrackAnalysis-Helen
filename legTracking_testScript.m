...
% for debugging - get mean velocity during each step
for i = 1:length(legInd)
...
%% determine length of each half step, in xy plane (not in 1 axis)

% length for each half step, as defined by start and end pts
% preallocate
for i = 1:length(legInd)
    stepLengths.front2Back.(legNames{legInd(i)}) = ...
        zeros(size(stepInd.front2Back.(legNames{legInd(i)}),1),1);
    stepLengths.back2Front.(legNames{legInd(i)}) = ...
        zeros(size(stepInd.back2Front.(legNames{legInd(i)}),1),1);
end
% compute length of each step for each leg
for i = 1:length(legInd)
    stepLengths.front2Back.(legNames{legInd(i)}) = distBtw2Pts(...
        legXShiftRotNorm(stepInd.front2Back.(legNames{legInd(i)})(:,1),...
        legInd(i)), ...
        legYShiftRotNorm(stepInd.front2Back.(legNames{legInd(i)})(:,1),...
        legInd(i)), ...
        ...
        legYShiftRotNorm(stepInd.front2Back.(legNames{legInd(i)})(:,2),...
        legInd(i)));
    stepLengths.back2Front.(legNames{legInd(i)}) = distBtw2Pts(...
        legXShiftRotNorm(stepInd.back2Front.(legNames{legInd(i)})(:,1),...
        legInd(i)), ...
        legYShiftRotNorm(stepInd.back2Front.(legNames{legInd(i)})(:,1),...
        legInd(i)), ...
